...
export const pincodeSchema = z.string().regex(/^\d{6}$/, 'Please enter a valid 6-digit pincode')

export const gstinSchema = z
  .string()
  .regex(/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/, 'Please enter a valid GSTIN')

export const hsnCodeSchema = z.string().regex(/^\d{4,8}$/, 'HSN code must be 4-8 digits')
